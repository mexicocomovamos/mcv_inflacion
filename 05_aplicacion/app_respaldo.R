...
gen_grafica <- function(genericos, 
    ...
    if(usar_leyenda){
        # Usar leyenda para más de 6 elementos
        g <- fyvs %>% 
            ggplot(aes(x = fecha, 
                       y = values, 
                       group = ccif, 
            ...
            labs(
                ...
                subtitle = subtitulo, 
                caption = nota,
                color="Genéricos", shape="", y = eje_y
            ) +
            theme(plot.title = element_text(size = 36, face = "bold", colour = "#6950D8"),
                  plot.title.position = "plot",
                  plot.subtitle = element_text(size = 30, colour = "#777777", margin=margin(0,0,20,0)),
                  plot.caption = element_text(size = 26, colour = "#777777"),
        ...
        # Usar etiquetas para 6 o menos elementos
        g <- fyvs %>% 
            ggplot(aes(x = fecha, 
                       y = values, 
                       group = ccif, 
...
gen_barras_cambio_anual <- function(genericos, fecha_inicio = "2018-12-01", 
    ...
    # Ajustar tamaño de letra y mostrar etiquetas según número de elementos
    mostrar_etiquetas <- length(genericos) <= 15
    
    tamanio_letra <- case_when(length(genericos) <= 1 ~ 12,
                               between(length(genericos), 2,3)  ~ 10,
                               between(length(genericos), 4,10)  ~ 8,
                               between(length(genericos), 11, 15) ~ 6,
                               TRUE ~ 0  # No mostrar etiquetas si son más de 15
    )
    ...
    # Añadir etiquetas solo si no son demasiados elementos
    if(mostrar_etiquetas && tamanio_letra > 0){
        grafica_base <- grafica_base + 
            geom_text(aes(label = str_c(round(tasa*100, 2), "%"), 
                      ...
                      fontface = "bold", 
                      family = "Arial") + 
            scale_discrete_manual(aesthetics = "hjust", values = c("1" = -0.1, "-1" = 1.1))
    }
    ...
    grafica_base +
        theme(plot.title = element_text(size = 36, face = "bold", colour = "#6950D8"),
              ...
              axis.title.x = element_blank(),
              axis.title.y = element_text(size = 30),
              axis.text.x = element_text(size = 26, angle = 0, vjust = 0.5),
              axis.text.y = element_text(size = ifelse(length(genericos) > 20, 18, 26)),
              legend.text = element_text(size = 26),
              legend.position = "none")
