...
prod_subyacentes <- tribble(
    ~nombre,                                         ~code,
    "pan de caja",                                  "009",
    "tortilla de maíz",                             "014",
    "leche pasteurizada y fresca",                  "034",
    "aceites y grasas vegetales comestibles",       "042",
    "jugos o néctares envasados",                   "095",
    "vivienda propia",                              "139",
    "automóviles",                                  "206",
    "transporte aéreo",                             "227",
    "servicios turísticos en paquete",              "255",
    "loncherías, fondas, torterías y taquerías",    "265",
...
unique(ident_suby$code) 


prod_nosubyacentes <- tribble(
    ~nombre,                         ~code,
    "carne de res",                  "018",
    "pollo",                         "022",
    "huevo",                         "031",
...
unique(ident_nosuby$code) 


NewVersion <- read_excel("01_datos_crudos/01_03_inpc_complete_NewVersion.xlsx")%>%
    mutate(
        nombre = str_to_lower(ccif),
        ponderador = ponderador_inpc_id_ccif_4,
        has_values = !is.na(ponderador) & !is.na(encadenamiento)
    ) %>%
    arrange(nombre, desc(has_values)) %>%  # put rows with actual values first
    group_by(nombre) %>%
    slice(1) %>%  # keep only the first row per nombre
    ungroup() %>%
    select(nombre, ponderador, encadenamiento)

ident_all <- bind_rows(ident_suby, ident_nosuby)%>%
    left_join(NewVersion, by = "nombre")%>%
...
d_monitoreo <- d_monitoreo%>% 
    mutate(nombre = firstup(nombre))
...
pegar_logo <- function(x){
    paste0(
        ...
        "![", x, "]", 
        # URL
        "(https://raw.githubusercontent.com/mexicocomovamos/mcv_inflacion/main/00_documentacion/00_iconos/", 
        x, ".png)")}

# Guardar nombres productos en orden alfabético 
v_productos <- sort(unique(d_monitoreo$nombre))
